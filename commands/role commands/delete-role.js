...
module.exports = {
  data: new SlashCommandBuilder()
    .setName("delete-role")
    .setDescription(`Delete a role to a member`)
    .addUserOption((option) =>
      option
        .setName("user")
        .setDescription("The user to delete a role from")
        .setRequired(true)
    )
    .addRoleOption((option) =>
      option
        .setName("role")
        .setDescription("The role to delete from the user")
        .setRequired(true)
    ),
      ...
      if (member.roles.cache.some((r) => r.id === existing[i].role_id)) {
        ...
        if (split.includes(role.id)) {
          ...
          const Embed = new EmbedBuilder()
            ...
            .setThumbnail(interaction.guild.iconURL())
            .setDescription(
              `> Dear ${interaction.member}, you have successfully removed ${role} from ${member}. The role that allowed you to perform this action was <@&${existing[i].role_id}>.`
            )
            .setTimestamp()
            .setAuthor({
              name: `${interaction.client.user.username}`,
